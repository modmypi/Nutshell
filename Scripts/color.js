...
* @author Stoyan Stefanov <sstoo@gmail.com>, with modifications by Bertrand Le Roy
* @link   http://www.phpied.com/rgb-color-parser-in-javascript/
* @license Use it if you like it
*/
var Color = function (colorString) {
    var ok = false;
    if (typeof colorString === "object" && Color.isColor(colorString)) {
        this.red = colorString.red;
        this.green = colorString.green;
        this.blue = colorString.blue;
        ok = true;
    }
        ...
        if (!ok) {
            ...
            for (var i = 0; i < colorDefs.length; i++) {
                var re = colorDefs[i].re,
                    ...
                    colorBits = re.exec(colorString);
                if (colorBits) {
                    var channels = processor(colorBits);
                    this.red = channels[0];
    ...
    this.toHex = function () {
        return Color.hex(this);
    };
    this.toHtml = function () {
        return Color.html(this);
    };
};
(function () {
    var extensions = {
        isColor: function (obj) {
            return obj && typeof obj.red === "number" && typeof obj.green === "number" && typeof obj.blue === "number";
        },
        ...
        hex: function (color) {
            return ("00000" + (color.red * 65536 + color.green * 256 + color.blue).toString(16)).slice(-6);
        },
        html: function(color) {
    ...
    for (var n in extensions) {
        var c = extensions[n];
        Color[n] = typeof c === "string" ? new Color(c) : c;
    }
})();